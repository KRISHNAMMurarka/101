...
Vision Lab and camera-enabled games use bundled pose and hand models locally. Face/head tasks and worker-based inference are separate vision work. The Node local Hub now supplies the tested signaling contract; the Tauri wrapper, native sensor collection, watches, and specialist hardware adapters build on it without changing game code.
