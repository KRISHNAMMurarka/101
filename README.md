...
`apps/controller-native` is the independently buildable **101 Link native app** for iOS and Android. It scans local pairing QR codes, reconnects with a durable device identity, uses two native WebRTC DataChannels, renders any validated host-supplied controller layout, includes classic/gamepad/wand/steering/tilt/touch/trigger/detector/sensor-lab presets, performs quaternion calibration and gesture recognition locally, and supports host haptics. It declares no microphone use. See [native Link development](docs/native-link.md).
...
The **Controller Lab** validates editable controller-layout JSON, applies it live to 101 Link, and inspects the normalized actions, axes, and vectors returned by buttons, shoulders, analog triggers/buttons, D-pads, tuned sticks, touch surfaces, sliders, and optional motion mappings.
